...
Reviewers HeVs (w2, w5) and yLdq (w1) all press on external validity: does
anything here survive outside 64x64 toy renders?  The 128x128 datasets double
every length, so the images are geometrically identical to the paper's and
DiT-S/4 at 128 sees the same 32x32 token grid DiT-S/2 sees at 64.  Resolution
is therefore the only variable.
...
def main():
    ...
    # Repoint to the rebuttal tree and scale the length-based thresholds.
