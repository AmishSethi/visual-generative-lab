"""Visually complex VGL renders (Reviewer HeVs, weakness 5).

HeVs objects that the paper "does not provide experiments that reduce this gap,
such as higher-resolution scenes, more realistic objects".  The 128x128 runs
cover resolution; this covers appearance.
...
def _apply_shape(background, shape_masks, size, rng):
    ...
    A single shared colour field made adjacent objects merge into one blob under
    the saturation key, which capped the count metric at 45% on ground truth --
    the metric, not the model, would have been doing the failing.
    """
...
def object_mask(image_rgb):
    ...
    which dropped the ground-truth count ceiling to 43%.  Without it the ceiling
    is 100% and single-object IoU is unchanged.
    """
