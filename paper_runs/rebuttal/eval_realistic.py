#!/usr/bin/env python
"""Evaluate the visually complex runs (Reviewer HeVs w5).
...
`--validate` scores ground-truth renders with the identical code first and
...
from paper_runs.rebuttal.realistic import (
        ...
        for value in values:
            ...
            for sample in samples[value]:
                ...
                if skill == "size":
                    ...
                    # evaluate_table2.py does.  int(round(v)) banker's-rounds the
                    # interpolation midpoints [2.5,3.5,4.5,5.5,6.5] to [2,4,4,6,6]
                    # -- all trained values -- so the interp split silently
                    # re-measured training accuracy and reported a spurious 43.0.
