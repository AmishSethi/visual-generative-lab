#!/usr/bin/env python
"""Evaluate rebuttal families that reuse the paper's metrics unchanged.

  ranges     narrower / shifted training supports (Reviewer yLdq w2).  Each run
             has its own dataset, so the train/interp grids come from that
             run's metadata while the extrapolation grid is widened to a common
             span -- otherwise each model would be probed over a different
             range and the break points would not be comparable.

  text_cond  frozen text-encoder conditioning (Reviewer HeVs w1/w6), trained on
             the paper's canonical datasets, so only the conditioning differs.
