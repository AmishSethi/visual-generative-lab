...
Reviewer HeVs (w1, w6): the paper's numeric conditioning "differ[s] from many
real-world settings", and it is "not fully clear how this maps to real-world
generation".  The direct answer is to condition VGL exactly the way a
text-to-image model does -- on a frozen text encoder's output for a natural
language description of the skill value -- and see whether the generalisation
picture changes.

Embeddings are precomputed here (in the env that has transformers) and saved as
a lookup table, so training and evaluation stay in the DiT env and never need to
run CLIP.  Out-of-range values get embeddings the same way in-range ones do, by
encoding their sentence, so extrapolation is a property of the language
representation rather than of an extrapolating projection layer.
"""
import os as _os
...
def build(skill, out_path, device):
    ...
    # linear embedder and is itself part of the answer.
...
def main():
    ...
    parser.add_argument("--out-dir", type=Path,
                        default=Path(f"{VGL_ROOT}/REBUTTAL/text_tables"))
