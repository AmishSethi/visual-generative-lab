#!/usr/bin/env python
"""Generate and visually inspect samples from every finished appendix family.

Rule-based extractors can agree with each other and still be wrong about what
the model produced -- a counter that misses objects, or an interpolation split
whose queries round onto trained values -- and inspecting the images catches
such errors.
...
FAMILIES = {
    "realistic-size":     ("realistic/size",      APPENDIX/"datasets/realistic/size",   "size",     "extra"),
    "realistic-rotation": ("realistic/rotation",  APPENDIX/"datasets/realistic/rotation","rotation", "extra"),
    "realistic-count":    ("realistic/count",     APPENDIX/"datasets/realistic/count",  "count",    "train"),
    "hires128-size":      ("hires128/size",       APPENDIX/"datasets/hires128/size",    "size",     "extra"),
    "hires128-rotation":  ("hires128/rotation",   APPENDIX/"datasets/hires128/rotation","rotation", "extra"),
...
def score(skill, image, value, realistic):
    ...
    if realistic:
        from paper_runs.appendix import eval_realistic as er
        if skill == "size":
    ...
    if skill == "rotation":
        ...
        # 128x128 image must be downscaled first -- exactly what eval_hires.py in
        # this directory does.  Skipping this flips head-for-tail and yields
        # 180-degree errors.
        if image.shape[0] != 64:
...
def main():
    ...
    ap.add_argument("--out-dir", type=Path, default=APPENDIX/"eval"/"visual_inspection")
    a = ap.parse_args()
    ...
    for name in a.families:
        ...
        t2.RESULTS_ROOT = APPENDIX/"results"/Path(sub).parent
        if realistic:
            t2.DATASET_ROOT = APPENDIX/"datasets/realistic"
