#!/usr/bin/env python
"""Separate coverage fraction from number of combinations.

In the paper's 4-shape x 8-colour grid, coverage fraction and the number of
unique training combinations K move together, so a "coverage matters" result
could equally be "more unique combinations matter".  Varying both
vocabularies breaks that:
...
def matched_contrasts(cells):
    ...
    if not any_shown:
        print("  (no K value has two different coverages evaluated)")
