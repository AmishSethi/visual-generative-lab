...
pixel-versus-latent gap.  If it stays near the pixel baseline, the representation (VAE
compression) matters rather than the sequence length.
"""
import argparse
import json
import sys
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=APPENDIX / "eval" / "token_matched_results.json")
    a = ap.parse_args()
    ...
    # Datasets live in the paper tree; checkpoints live under the appendix tree.
    t2.DATASET_ROOT = T2 / "datasets"
    t2.RESULTS_ROOT = APPENDIX / "results" / "token_matched"
    ...
    for tag in a.tags:
        ...
        target = APPENDIX / "eval" / "token_matched" / tag
