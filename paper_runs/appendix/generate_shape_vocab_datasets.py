#!/usr/bin/env python
"""Shape x colour datasets for the coverage-vs-K experiment.

The "coverage matters more than dataset size" result could instead be a
"number of unique combinations" result: raising the coverage fraction also
raises the absolute number of combinations K, and with only 4 shapes the two
are locked together.  Doubling the shape vocabulary unlocks them:
...
from paper_runs.appendix.manifest import DATASET_ROOT
from paper_runs.appendix.shapes8 import COLORS, COLORS16, SHAPES4, SHAPES8, render_shape
...
def main():
    ...
    parser.add_argument("--mask-seeds", nargs="+", type=int, default=None,
                        help="Emit one dataset per seed into shape_vocab_masks/ to measure "
                             "variance across random coverage masks.")
